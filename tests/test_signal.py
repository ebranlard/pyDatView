...
class TestSignal(unittest.TestCase):
    ...
    def test_up_down_sample(self):
        ...
        np.testing.assert_array_equal(x,[0.5, 2.5])
        np.testing.assert_array_equal(y,[2.5, 2.5])
        x, y = applySampler(range(0, 3), [5, 0, 5], {'name': name, 'param': [0.5]})
        np.testing.assert_array_equal(x,[0, 0.5, 1, 1.5, 2])
        np.testing.assert_array_equal(y,[5, 2.5, 0, 2.5, 5])
        x, df = applySampler(range(0, 6), None, {'name': name, 'param': [3]}, pd.DataFrame({"y": [0, 6, 6, 2, -4, -4]}))
        np.testing.assert_array_equal(x,[1, 4])
        np.testing.assert_array_equal(df["y"],[4, -2])
        x, df = applySampler(range(0, 3), None, {'name': name, 'param': [0.5]}, pd.DataFrame({"y": [0, 6, -6]}))
        np.testing.assert_array_equal(x,[0, 0.5, 1, 1.5, 2])
        np.testing.assert_array_equal(df["y"],[0, 3, 6, 0, -6])
